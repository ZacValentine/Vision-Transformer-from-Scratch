...
# Vi-T Base parameters: layers: 12, embed_dim: 768, ff_dim: 3072, heads: 12, parameters: 86M
# patch size not mentioned
# batch size: 4096
...
# default values to 0 when there's no checkpoint to load from
initial_epoch = 0
initial_i = 0

# path to save weights and training stats throughout training
checkpoint_weights_path = "saves/checkpoint_model_weights.pkl"
checkpoint_stats_path = "saves/checkpoint_training_stats.json"

# path to load model from, not necessary if training without loading from checkpoint
saved_weights_path = "saves/saved_model_weights.pkl"
saved_stats_path = "saves/saved_training_stats.json"

# determines if model is loaded from checkpoint before training, could be put as command line argument
load_from_checkpoint = True

# loads or initializes trainable parameters and training stats depending on value of load_from_checkpoint
if load_from_checkpoint:
    # load model and training stats from saved files
    with open(saved_weights_path, "rb") as f:
        loaded_weights = pickle.load(f)

    with open(saved_stats_path, 'r') as f:
        training_stats = json.load(f)

    # load each trainable parameter from save
    linear1 = loaded_weights["linear1"]
    bias1 = loaded_weights["bias1"]
    position_embeddings = loaded_weights["position_embeddings"]
    cls_token = loaded_weights["cls_token"]
    linear4 = loaded_weights["linear4"]
    bias4 = loaded_weights["bias4"]
    linear5 = loaded_weights["linear5"]
    bias5 = loaded_weights["bias5"]
    layers = loaded_weights["layers"]

    # starting epoch and iteration are the final epoch and iteration from the saved stats, effectively resuming training where it left off
    # initial iteration is incremented by 1 to not repeat final trained example
    initial_epoch = training_stats["epochs"][-1]
    initial_i = training_stats["iterations"][-1] + 1
